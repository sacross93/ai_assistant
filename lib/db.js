...
// Ensure OCR agent exists (migration for existing databases)
const ocrAgent = db.prepare("SELECT id FROM agents WHERE id = 'ocr'").get();
if (!ocrAgent) {
  const maxOrder = db.prepare('SELECT MAX(order_index) as max_idx FROM agents').get();
  const nextOrder = (maxOrder?.max_idx ?? -1) + 1;
  db.prepare(
    'INSERT OR IGNORE INTO agents (id, name, description, features, order_index, is_active) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(
    'ocr',
    'OCR 텍스트 추출',
    '이미지나 PDF 파일에서 텍스트를 자동으로 추출합니다. 스캔된 문서나 사진 속 텍스트를 인식하여 편집 가능한 텍스트로 변환합니다.',
    JSON.stringify(['이미지/PDF 텍스트 인식', '다중 파일 동시 처리', 'Markdown/JSON 출력 지원']),
    nextOrder,
    1
  );
  console.log('Inserted OCR agent into existing agents table.');
}
